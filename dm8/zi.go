...
func (G2DB g2db) fromInt64(val int64, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case BIGINT:
		...
		return Dm_build_931.Dm_build_1121(val), nil
	case REAL:
...
func (G2DB g2db) fromFloat64(val float64, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case REAL:
		...
		return Dm_build_931.Dm_build_1124(float32(val)), nil
	case DOUBLE:
		return Dm_build_931.Dm_build_1127(val), nil
	case DECIMAL:
...
func (G2DB g2db) toChar(bt byte) ([]byte, error) {
	...
	b[0], err = G2DB.getCharByNumVal((bt >> 4) & 0x0F)
	if err != nil {
		return nil, err
	}

	b[1], err = G2DB.getCharByNumVal(bt & 0x0F)
	if err != nil {
		return nil, err
	}
...
func (G2DB g2db) fromDecimal(val DmDecimal, param parameter, conn *DmConnection) ([]byte, error) {
	...
	case BIGINT:
		...
		ret = Dm_build_931.Dm_build_1121(val.ToBigInt().Int64())
	case REAL:
...
func (G2DB g2db) fromBlob(val DmBlob, param parameter, conn *DmConnection) (interface{}, error) {
	...
	switch param.colType {
	case BINARY, VARBINARY:
		length, err := val.GetLength()
		if err != nil {
			return nil, err
		}
		ret, err = val.getBytes(1, int32(length))
		if err != nil {
			return nil, err
		}
	case BLOB:
...
func (G2DB g2db) fromClob(val DmClob, param parameter, conn *DmConnection) (interface{}, error) {
	...
	case CHAR, VARCHAR, VARCHAR2:
		...
		s, err = val.getSubString(1, int32(length))
		if err != nil {
			return nil, err
		}
		ret = []byte(s)
...
func (G2DB g2db) toInt32(val int32) []byte {
	b := make([]byte, 4)
	Dm_build_931.Dm_build_947(b, 0, val)
	return b
}

func (G2DB g2db) toInt64(val int64) []byte {
	b := make([]byte, 8)
	Dm_build_931.Dm_build_952(b, 0, val)
	return b
}

func (G2DB g2db) toFloat32(val float32) []byte {
	b := make([]byte, 4)
	Dm_build_931.Dm_build_957(b, 0, val)
	return b
}

func (G2DB g2db) toFloat64(val float64) []byte {
	b := make([]byte, 8)
	Dm_build_931.Dm_build_962(b, 0, val)
	return b
}
