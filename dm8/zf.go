...
// DmConnectorConnect DmConnector
...
// DmConnectionBegin DmConnection
...
func (rf *reconnectFilter) DmStatementNumInput(filterChain *filterChain, s *DmStatement) int {
	...
	defer func() {
		err := recover()
		if err != nil {
			_ = rf.autoReconnect(s.dmConn, err.(error))
			ret = 0
		}
	}()
