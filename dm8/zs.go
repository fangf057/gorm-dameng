...
var monNameList = []string{"", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"}
...
func (OracleDateFormat *oracleDateFormat) setPattern(pattern string) {
	if pattern != OracleDateFormat.pattern {
		...
		_, _ = OracleDateFormat.analysePattern(pattern)
	}
}
				...
				for ; i > 0; i-- {
					...
					if element != nil {
						...
						if element == OracleDateFormat.TZHElement || element == OracleDateFormat.TZMElement {
							var lastFormatElement = OracleDateFormat.FormatElementList[len(OracleDateFormat.FormatElementList)-1].(string)
